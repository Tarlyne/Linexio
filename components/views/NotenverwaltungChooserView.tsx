...
const NotenverwaltungChooserView: React.FC = () => {
  const { lerngruppen } = useLerngruppenContext();
  const { currentSchoolYear, onShowNotenverwaltung, setHeaderConfig } = useUIContext();
    ...
    <div className="max-w-7xl">
      {filteredLerngruppen.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredLerngruppen.map((lg: Lerngruppe) => (
            <LerngruppeCard
              ...
              onSelect={() => onShowNotenverwaltung(lg.id)}
              onEdit={() => {}}
              onMoveUp={() => {}}
              onMoveDown={() => {}}
              isFirst={true}
        ...
        <div className="text-center p-12 bg-[var(--color-ui-primary)] rounded-lg border border-[var(--color-border)] max-w-2xl mx-auto mt-10">
          ...
          <p className="text-[var(--color-text-tertiary)] mt-2">Sie müssen zuerst eine Lerngruppe erstellen, um Noten verwalten zu können.</p>
        </div>
      )}
    </div>
  );
...
export default NotenverwaltungChooserView;