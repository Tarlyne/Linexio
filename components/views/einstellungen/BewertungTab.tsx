...
interface NotenschluesselTabProps {
    ...
    // FIX: Update prop type to expect a function that returns a Promise.
            ...
            <div className="flex w-full max-w-md bg-[var(--color-ui-secondary)] rounded-lg p-1 space-x-1">
                {notensysteme.map(ns => (
                    <button
                        ...
                        className={`flex-1 p-2 text-sm font-semibold rounded-md transition-colors ${activeSystemId === ns.id ? 'bg-gradient-to-r from-[var(--color-accent-primary-hover)] to-[var(--color-accent-primary)] text-[var(--color-text-primary)]' : 'text-[var(--color-text-secondary)] hover:bg-[var(--color-ui-tertiary)]'}`}
                    >
                        {ns.name}
                    </button>
                ))}
